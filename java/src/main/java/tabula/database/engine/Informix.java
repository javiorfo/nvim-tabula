package tabula.database.engine;

import lombok.AllArgsConstructor;
import lombok.Getter;
import tabula.database.engine.model.ProtoSQL;
import tabula.database.factory.Executor;

@AllArgsConstructor
@Getter
public class Informix implements Executor {
    private ProtoSQL protoSQL;
    ...
    public void getTableInfo() {
        protoSQL.setQueries(this.getTableInfoQuery(protoSQL.getQueries()));
        protoSQL.executeSelect(protoSQL.getConnection());
    }
    ...
    private String getTableInfoQuery(String tableName) {
        return """
                    SELECT
                    ...
                    FROM
                        systables AS tab
                    JOIN
                        syscolumns AS col ON tab.tabid = col.tabid
                    LEFT JOIN
                        sysindexes AS idx ON col.tabid = idx.tabid AND col.colno = idx.part1
                    LEFT JOIN
                        sysconstraints AS cons ON idx.idxname = cons.idxname AND tab.tabid = cons.tabid
                    LEFT JOIN
                        sysreferences AS sr ON cons.constrid = sr.constrid
                    LEFT JOIN
                        systables AS srtab ON srtab.tabid = sr.ptabid
                    WHERE
                     tab.tabname = '%s';
                """
                .formatted(tableName);
